...
from api.notifications_setting import send_user_notification
...
def save_payment(
    ...
    if status == "success":
        ...
        send_user_notification(
            ...
            message=f"Your {plan.upper()} plan is now active.",
        ...
        try:
            ...
            print(f"❌ Failed to send payment email (API): {e}")
...
async def razorpay_webhook(request: Request):
    ...
    if event == "payment.captured":
        ...
        # Check if already processed via API
        existing = col_payments.find_one({"payment_id": payment_id})

        # If DB record missing or mail not sent, process it
        ...
        # SEND NOTIFICATION & EMAIL IF NOT DONE
        if existing and existing.get("user_id") and not mail_already_sent:
            uid = normalize_user_id(existing["user_id"])
            ...
            send_user_notification(
                ...
                notif_type="payment_success",
                title="Payment Successful",
                message=f"Your {existing.get('plan_name', 'Premium')} plan is active.",
                related_id=payment_id
            )
            ...
            try:
                send_payment_success_mail(
                    user_id=uid,
                    plan_name=existing.get("plan_name", "Plan"),
                    amount=amount,
        ...
        if payment:
            ...
            new_expiry = datetime.utcnow() + timedelta(days=PLAN_CONFIG[plan]["days"])
